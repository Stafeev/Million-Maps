...
var ACTIVE_TRIP_PLAN_UUID = "undefined";
...
$(document).ready(function() {
    ...
    $("#select_trip_plan").bind( "change", function(event, ui) {
        ACTIVE_TRIP_PLAN_UUID = $("#select_trip_plan").val();
	...
	$("#select_map").bind( "change", function(event, ui) {
        ACTIVE_MAP = $("#select_map").val();
		localStorage.activeMap = $("#select_map").val();
        loadMap();
    });
		
	if(ACTIVE_MAP !== "undefined") {
                $("#select_map").val(ACTIVE_MAP).attr('selected', true).siblings('option').removeAttr('selected');
                //$("#select_map").selectmenu("refresh");
            };
...
function deleteTripPlanClicked(uuid) {
    ...
    function deleteTripPlanClickedSuccessCB() {
        ACTIVE_TRIP_PLAN_UUID = "undefined";
        if(typeof(Storage)!=="undefined") {
...
function loadTripPlansToSelect() {
    function loadTripPlansToSelectSuccessCB(tripPlanList) {
        if(tripPlanList.length < 1) {
        	$("#select_trip_plan").trigger("create");
            ...
            if(ACTIVE_TRIP_PLAN_UUID !== "undefined") {
                $("#select_trip_plan").val(ACTIVE_TRIP_PLAN_UUID).attr('selected', true).siblings('option').removeAttr('selected');
                $("#select_trip_plan").selectmenu("refresh");
            }
            else {
...
function facebookShare() {
    ...
    var linkURL = "http://vresorts.com/FTBeta/tripPlanner/sharedTripPlan.html?trip_plan_uuid::";
    window.location.href = facebookURL + linkURL + ACTIVE_TRIP_PLAN_UUID;
}
