...
function anonymousLoginClicked() {
    $.getJSON(window.globalURL + "/anonymousLogin?device_id=" + device.uuid + "&" + window.apikey, function(anonymousUser){
...
//load plans for map view, solve the problem for early refreshing listview
function loadPlansForMap()
{
    function loadPlansSuccess(tripPlanList)
    {
        ...
        if (!loaded)
        {
            ...
            requestTripPlan(window.globalURL + '/getPlaces?trip_plan_uuid='+myPlan + "&" + window.apikey, function(tripplan){
