...
//    	}
        ...
        placesList.forEach(function(place) {
            if(place.offer_uuid != "0") {
                ...
                var offerDetail = 
                    "<a href='#' data-role='button' data-mini='true' style='width:4em;' "
                    + "onclick=\x22offerDetailsClicked(\x27"
                    + place.offer_uuid 
                    + "\x27);\x22>Details</a>";
                var switchHTML =
                    "<div class='containing-element'> <select onchange=\x22togglePlaceSubscription(\x27"
                    + place.uuid
                    + "\x27);\x22"
                    + "name='switch-" + place.uuid + "' "
                    + "id='switch-" + place.uuid + "' data-role='slider'  style='margin:15px; width:9em'>"
